...
function thumb($src_file, $width = false, $height = false, $crop = false, $format = false) {
	...
	// get target folder
	$pos2 = strrpos($src_file,  DIRECTORY_SEPARATOR);
    $folder = substr($src_file, 0 , $pos2);
	
    $thumb_url = substr($src_file, 0, strpos($src_file,  DIRECTORY_SEPARATOR));
    ...
    $thumb_file = $folder .(preg_match('/:/',$thumb_url)?'':$thumb_url)  . DIRECTORY_SEPARATOR . "__{$width}__{$height}__{$crop}__{$last_modified}.{$format}";

    // if exist before then return the image
    if (file_exists($thumb_file))
        return substr($thumb_file, strrpos($thumb_file, 'media'));
