...
function thumb($src_file, $width = false, $height = false, $crop = false, $format = false) {
	...
	// get target folder
	$pos2 = strrpos($src_file,  "/");
    $folder = substr($src_file, 0 , $pos2);
	
    $thumb_url = substr($src_file, 0, strpos($src_file,  "/"));
    ...
    $thumb_file = str_replace("\\", "/", $folder) .(preg_match('/:/',$thumb_url)?'':$thumb_url)  . "/" . "__{$width}__{$height}__{$crop}__{$last_modified}.{$format}";
